...
async function getAccessToken(): Promise<string> {
  ...
  if (!clientId || !clientSecret || !tokenUrl) {
    throw new Error(
      "Missing required environment variables. Set LYTHO_CLIENT_ID, " +
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Lytho token request failed ${response.status} ${response.statusText}: ${body}`
    );
  }
...
const RESOURCE_TYPES = ["workrequest", "task", "proof", "project", "campaign"] as const;
const DESCRIBABLE_TYPES = ["task", "proof", "project", "campaign"] as const;
...
async function lythoFetch(path: string, options: RequestInit = {}): Promise<unknown> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Lytho API ${response.status} ${response.statusText}: ${body}`
    );
  }
...
server.registerTool(
  ...
  {
    description:
      "Search Lytho items (work requests, tasks, proofs, projects, or campaigns) by name and archived status.",
    inputSchema: {
  ...
  async ({ type, name, isArchived, operatorMode }) => {
    ...
    if (isArchived !== undefined) params.set("isArchived", String(isArchived));
    if (operatorMode !== undefined) params.set("operatorMode", String(operatorMode));
...
server.registerTool(
  ...
  {
    description:
      "Update a Lytho item using JSON Patch operations (RFC 6902). Use a 'test' operation first to guard against concurrent edits, then 'replace' to change fields.",
    inputSchema: {
      type: resourceTypeSchema,
      id: idSchema,
      operations: z
...
server.registerTool(
  ...
  {
    description:
      "Get downloadable file URLs attached to a Lytho item. Work requests return deliverable file URLs; all other types return general file URLs.",
    inputSchema: {
      type: resourceTypeSchema,
      id: idSchema,
    },
...
server.registerTool(
  ...
  {
    description:
      "Get the rich-text description for a Lytho task, proof, project, or campaign. Not available for work requests.",
    inputSchema: {
...
server.registerTool(
  ...
  {
    description:
      "Batch-retrieve descriptions for multiple Lytho tasks, proofs, projects, or campaigns. More efficient than calling get_description repeatedly.",
    inputSchema: {
      type: describableTypeSchema,
...
server.registerTool(
  ...
  {
    description:
      "Retrieve Lytho user preferences. Provide a single 'id' for one preference, " +
  ...
  async ({ id, ids, page, pageSize }) => {
    ...
    const params = new URLSearchParams();
    if (ids?.length) {
      for (const pid of ids) params.append("ids", String(pid));
    }
    if (page) params.set("page", String(page));
    if (pageSize) params.set("pageSize", String(pageSize));
